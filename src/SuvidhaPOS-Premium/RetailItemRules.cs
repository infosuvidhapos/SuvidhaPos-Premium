    ...
    internal static async Task<IResult> Save(Db db,int? id,global::ProductRequest x)
    {
        ...
        try{
            ...
            using var cmd=Command(c,tx,id.HasValue?@"UPDATE Products SET Name=@n,Barcode=@b,Sku=@sku,CategoryId=@cid,Category=@cat,Unit=@unit,Hsn=@hsn,GstRate=@gst,TaxMode=COALESCE(@mode,TaxMode),Mrp=@mrp,PurchasePrice=@cost,SalePrice=CASE WHEN @disc IS NULL THEN SalePrice ELSE @sale END,Dis_Rate=COALESCE(@disc,Dis_Rate),MinStock=@min,MaxStock=@max,LocationCode=@loc,RackName=@rack,ShelfName=@shelf,TrackBatch=@batch,TrackExpiry=@expiry WHERE Id=@id;SELECT @id":@"INSERT Products(Name,Barcode,Sku,CategoryId,Category,Unit,Hsn,GstRate,TaxMode,Mrp,PurchasePrice,SalePrice,Dis_Rate,MinStock,MaxStock,LocationCode,RackName,ShelfName,TrackBatch,TrackExpiry) OUTPUT INSERTED.Id VALUES(@n,@b,@sku,@cid,@cat,@unit,@hsn,@gst,COALESCE(@mode,'INCLUSIVE'),@mrp,@cost,@sale,COALESCE(@disc,0),@min,@max,@loc,@rack,@shelf,@batch,@expiry)",
                PurchasePostingService.P("@id",id),PurchasePostingService.P("@n",name),PurchasePostingService.P("@b",barcode),PurchasePostingService.P("@sku",string.IsNullOrWhiteSpace(x.Sku)?null:x.Sku.Trim()),PurchasePostingService.P("@cid",x.CategoryId),PurchasePostingService.P("@cat",x.Category),PurchasePostingService.P("@unit",unit),PurchasePostingService.P("@hsn",x.Hsn),PurchasePostingService.P("@gst",x.GstRate),PurchasePostingService.P("@mode",mode),PurchasePostingService.P("@mrp",x.Mrp),PurchasePostingService.P("@cost",x.PurchasePrice),PurchasePostingService.P("@sale",sale),PurchasePostingService.P("@disc",x.DiscountPer),PurchasePostingService.P("@min",x.MinStock),PurchasePostingService.P("@max",x.MaxStock),PurchasePostingService.P("@loc",x.LocationCode),PurchasePostingService.P("@rack",x.RackName),PurchasePostingService.P("@shelf",x.ShelfName),PurchasePostingService.P("@batch",x.TrackBatch),PurchasePostingService.P("@expiry",x.TrackExpiry));
...
WHEN NOT MATCHED THEN INSERT(ProductId,BaseUnit,InnerUnit,PackUnit,ConversionFactor,InnerConversionFactor,PackInnerFactor,PackPurchaseRate,PackMrp,PackSalePrice,InnerPurchaseRate,InnerMrp,InnerSalePrice,LooseSalePrice,AllowLoose) VALUES(@id,@b,@iu,@p,@f,@if,@pf,@cost,@mrp,@sale,@ic,@im,@is,@loose,@allow);",PurchasePostingService.P("@id",id),PurchasePostingService.P("@b",x.BaseUnit),PurchasePostingService.P("@iu",x.InnerUnit),PurchasePostingService.P("@p",x.PackUnit),PurchasePostingService.P("@f",x.ConversionFactor),PurchasePostingService.P("@if",x.InnerConversionFactor),PurchasePostingService.P("@pf",x.PackInnerFactor),PurchasePostingService.P("@cost",x.PackPurchaseRate),PurchasePostingService.P("@mrp",x.PackMrp),PurchasePostingService.P("@sale",x.PackSalePrice),PurchasePostingService.P("@ic",x.InnerPurchaseRate),PurchasePostingService.P("@im",x.InnerMrp),PurchasePostingService.P("@is",x.InnerSalePrice),PurchasePostingService.P("@loose",x.LooseSalePrice),PurchasePostingService.P("@allow",x.AllowLoose));await cmd.ExecuteNonQueryAsync();
    }
    internal static async Task<IResult> UpdateUom(Db db,int id,SpecializedModules.UomRequest x)
    {
        using var c=db.CreateConnection();await c.OpenAsync();using var tx=c.BeginTransaction(IsolationLevel.Serializable);
        try{await Lock(c,tx);var data=await PurchaseImportRules.Load(c,tx,true);var master=data.Masters.SingleOrDefault(m=>m.Id==id)??throw new InvalidOperationException("Product does not exist");var uom=NormalizeUom(x,data.Units);await CheckBaseChange(c,tx,id,master.BaseUnit,uom.BaseUnit);await SaveUom(c,tx,id,uom);using var cmd=Command(c,tx,"UPDATE Products SET Unit=@u WHERE Id=@id",PurchasePostingService.P("@u",uom.BaseUnit),PurchasePostingService.P("@id",id));await cmd.ExecuteNonQueryAsync();await tx.CommitAsync();return Results.Ok(new{saved=true,uom.BaseUnit,uom.InnerUnit,uom.PackUnit,uom.ConversionFactor,uom.InnerConversionFactor,uom.PackInnerFactor});}
        catch(Exception ex)when(ex is InvalidOperationException or SqlException or OverflowException){await tx.RollbackAsync();return Results.BadRequest(new{message=ex.Message});}
    }
...
FROM ProductUoms u JOIN Products p ON p.Id=u.ProductId WHERE p.Id=@id",PurchasePostingService.P("@id",id),PurchasePostingService.P("@purchase",purchase),PurchasePostingService.P("@mrp",mrp),PurchasePostingService.P("@sale",sale),PurchasePostingService.P("@discount",discount));await cmd.ExecuteNonQueryAsync();
    }
}
