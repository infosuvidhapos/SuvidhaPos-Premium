...
app.MapPut("/api/outlet",async(Db db,OutletRequest x)=>{
 ...
 var state=(x.State??"").Trim(); var city=(x.City??"").Trim();
 if(existing.Count==0) return Results.Ok(new{id=await db.ScalarAsync("INSERT OutletMaster(OutletName,StoreType,Address,Phone,Gstin,RequireBatch,RequireExpiry,DefaultUnit,State,City) VALUES(@n,@t,@a,@p,@g,@b,@e,@u,@st,@ct);SELECT CAST(SCOPE_IDENTITY() AS int)",P("@n",x.OutletName),P("@t",storeType),P("@a",x.Address),P("@p",x.Phone),P("@g",x.Gstin),P("@b",x.RequireBatch),P("@e",x.RequireExpiry),P("@u",x.DefaultUnit??"PCS"),P("@st",string.IsNullOrWhiteSpace(state)?null:state),P("@ct",string.IsNullOrWhiteSpace(city)?null:city))});
 var id=Convert.ToInt32(existing["Id"]); await db.ScalarAsync("UPDATE OutletMaster SET OutletName=@n,StoreType=@t,Address=@a,Phone=@p,Gstin=@g,RequireBatch=@b,RequireExpiry=@e,DefaultUnit=@u,State=@st,City=@ct,UpdatedAt=SYSDATETIME() WHERE Id=@id",P("@n",x.OutletName),P("@t",storeType),P("@a",x.Address),P("@p",x.Phone),P("@g",x.Gstin),P("@b",x.RequireBatch),P("@e",x.RequireExpiry),P("@u",x.DefaultUnit??"PCS"),P("@st",string.IsNullOrWhiteSpace(state)?null:state),P("@ct",string.IsNullOrWhiteSpace(city)?null:city),P("@id",id)); return Results.Ok(new{saved=true,id,storeType,state,city,storeTypeManagedBy="SuvidhaPremium"});
});
