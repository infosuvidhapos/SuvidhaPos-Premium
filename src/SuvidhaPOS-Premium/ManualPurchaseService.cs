...
internal static class ManualPurchaseService
{
    internal static async Task<IResult> Commit(Db db,global::PurchaseRequest x,string user)
    {
        ...
        try{
            ...
            if(x.SupplierId.HasValue){using var supplierCmd=Command(c,tx,"SELECT Name FROM Suppliers WHERE Id=@id AND IsActive=1",P("@id",x.SupplierId));var found=(await supplierCmd.ExecuteScalarAsync())?.ToString()??throw new InvalidOperationException("Supplier is inactive or missing");if(!string.IsNullOrWhiteSpace(x.SupplierName)&&PurchaseImportRules.Key(x.SupplierName)!=PurchaseImportRules.Key(found))throw new InvalidOperationException("Selected supplier does not match supplier name");supplier=found;}
