...
WHEN NOT MATCHED THEN
 ...
 P("@lp",x.LooseSalePrice),P("@lo",x.AllowLoose));
        ...
        app.MapGet("/api/jewellery/catalog", async (Db db, string? q) => Results.Ok(await db.QueryAsync(@"SELECT j.Id,j.TagNo,j.Barcode,j.ItemName,j.Category,j.DesignCode,j.MetalType,j.Purity,j.PurityPercent,j.HallmarkStatus,j.Huid,j.GrossWeight,j.LessWeight,j.NetWeight,j.FineWeight,j.WastagePercent,j.StoneWeight,j.StoneType,j.StonePieces,j.StoneCarat,j.StoneValue,j.MakingChargeType,j.MakingValue,j.LabourCharge,j.HallmarkCharge,j.OtherCharge,j.HsnCode,j.GstMode,j.GstRate,j.PurchasePrice,j.SalePrice,j.WholesalePrice,j.MinSalePrice,j.Status,j.LocationCode,j.RackName,j.TrayName,j.BoxName FROM JewelleryItems j WHERE j.Status IN ('IN_STOCK','APPROVAL','KARIGAR_WORK') AND (@q='' OR j.TagNo LIKE @l OR ISNULL(j.Barcode,'') LIKE @l OR j.ItemName LIKE @l OR ISNULL(j.Huid,'') LIKE @l) ORDER BY j.ItemName",P("@q",q??""),P("@l","%"+(q??"")+"%"))));
