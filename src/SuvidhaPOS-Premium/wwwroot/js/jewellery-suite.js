(function(){
  ...
  async function invoice(mode='retail'){
    ...
    appBox().innerHTML=`<div class="js-content js-invoice-page">
      ${pageHeading(name,'Create a new '+name.toLowerCase(),'<button class="js-outline" onclick="window.print()">▣ Print</button><button class="js-gold" onclick="jewelSuiteSaveInvoice()">▣ Save Bill</button>')}
  ...
  function lineCalc(x){
    ...
    const amount=Number(x.NetWeight||0)*r+Number(x.StoneValue||0)+making;
    return {r,fine,making,amount};
  }
  function renderBill(){
    const tb=document.getElementById('jsSaleRows');if(!tb)return;
    tb.innerHTML=JS.cart.length?JS.cart.map((x,i)=>{const z=lineCalc(x);return `<tr><td>${i+1}</td><td class="mono">${esc(x.TagNo)}</td><td>${esc(x.ItemName)}</td><td>${esc(x.MetalType)}</td><td>${esc(x.Purity)}</td><td>${money(x.GrossWeight)}</td><td>${money(Number(x.GrossWeight||0)-Number(x.NetWeight||0))}</td><td>${money(x.NetWeight)}</td><td>${money(x.PurityPercent)}%</td><td>0.00%</td><td>₹${money(z.making)}</td><td>${money(z.fine)}</td><td>₹${money(z.r)}</td><td><b>₹${money(z.amount)}</b></td><td><button onclick="jewelSuiteRemoveItem(${i})">×</button></td></tr>`}).join(''):'<tr><td colspan="15" class="js-empty">No items added yet<br><button onclick="jewelSuitePickItem()">＋ &nbsp; Add First Item</button></td></tr>';
    ...
    const pays=document.getElementById('jsPayRows');if(pays)pays.innerHTML=JS.payments.map((p,i)=>`<div class="js-payment-row"><select onchange="JSuitePayment(${i},'mode',this.value)"><option ${p.mode==='Cash'?'selected':''}>Cash</option><option ${p.mode==='Credit/UPI'?'selected':''}>Credit/UPI</option><option ${p.mode==='BTC'?'selected':''}>BTC</option><option ${p.mode==='Multi Mode'?'selected':''}>Multi Mode</option></select><input type="number" placeholder="Amount ₹" value="${p.amount||''}" onchange="JSuitePayment(${i},'amount',this.value)"><button onclick="jewelSuiteRemovePayment(${i})">⌫</button></div>`).join('');
  }
  ...
  window.jewelSuiteAddItem=function(id){const x=JS.items.find(a=>a.Id===id);if(x){JS.cart.push({...x,StoneValue:0});closeModal();renderBill()}};
