...
w.openBarcodePurchase=async function(mode='barcode'){
 ...
 app.innerHTML=`<div class="content retail-purchase barcode-purchase-full purchase-mode-${mode}"><div class="rp-heading"><div><span class="rp-eyebrow">${mode==='standard'?'STANDARD PURCHASE':'BARCODE PURCHASE · KEYBOARD READY'}</span><h2>${mode==='standard'?'New Purchase Entry':'Barcode Purchase Entry'}</h2><p>${mode==='standard'?'Standard supplier bill entry with unit, GST and discount controls.':'Scan barcode or search item name, use ↑↓ + Enter, then continue through the row with Enter.'}</p></div><button class="btn secondary" onclick="leavePurchaseEntry()">← Purchases</button></div>
 <div class="rp-keyboard-strip">${mode==='barcode'?'<span>F4 Item Search</span><span>↑↓ Select</span><span>Enter Next Field</span><span>F10 Save</span>':'<span>Bill No Required</span><span>Enter Next Field</span><span>F10 Save</span>'}</div><div class="panel rp-bill-head"><label>Bill No :<input id="bpi" class="input" maxlength="100" placeholder="Supplier bill number" onkeydown="purchaseHeaderKey(event,'bpdate')" oninput="purchaseDraftChanged()"></label><label>Bill Date :<input id="bpdate" class="input" type="date" value="${today()}" onkeydown="purchaseHeaderKey(event,'bpsup')" onchange="purchaseDraftChanged()"></label><label>Supplier Name :<select id="bpsup" class="select" onkeydown="purchaseHeaderKey(event,'bpScan')" onchange="purchaseDraftChanged()">${supplierOptions()}</select></label><div class="rp-draft-actions"><button class="btn secondary" onclick="barcodePurchaseTempSave()">Temp Save</button><button class="btn secondary" onclick="barcodePurchaseTempLoad()">Get Temp Data</button></div></div>
 ...
 <div class="rp-table-hint"><span>Prices are per selected unit · change the unit for box/strip purchase</span><span>F4 Search · F10 Save</span></div><div id="bpLines"></div><div id="bpQuickStats" class="rp-quick-stats"></div></div>
...
w.renderPurchaseTotals=function(){
 ...
 const quick=d.getElementById('bpQuickStats'),totalQty=rows.reduce((a,x)=>a+Number(x.Qty||0),0),billDiscount=Number(read('bpdisc'))||0;
 if(quick)quick.innerHTML=`<div><span>No. of Items</span><b>${rows.length}</b></div><div><span>Total Qty</span><b>${stockQty(totalQty)}</b></div><div><span>Bill Discount</span><b>₹${money(billDiscount)}</b></div><div><span>Taxable Amount</span><b>₹${money(sub)}</b></div><div><span>Total GST</span><b>₹${money(tax)}</b></div><div><span>Gross Amount</span><b>₹${money(total)}</b></div>`;
};
...
w.saveBarcodePurchase=async function(){
 if(saving)return;await scanQueue;if(saving)return;const lines=S.barPurLines||[];if(!lines.length)return toast('Add purchase items');const invoice=read('bpi').trim();if(!invoice)return alert('Bill No is required before saving purchase.');
