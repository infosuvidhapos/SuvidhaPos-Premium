(function(){
  ...
  function cbProductDiscount(p){return Math.max(0,Math.min(100,Number(p?.Dis_Rate||0)))}
  function cbGrossRate(p,choice){const disc=cbProductDiscount(p),sale=Math.max(0,Number(choice?.rate||0)),mrp=Math.max(0,Number(choice?.mrp||0));if(disc<=0)return sale;if(mrp>0)return mrp;if(disc<100&&sale>0)return sale/(1-disc/100);return sale}
  function cbLineDiscount(x){const raw=Math.max(0,Number(x?.Qty||0)*Number(x?.Rate||0)),percent=Math.max(0,Math.min(100,Number(x?.DiscountPer||0))),discount=raw*percent/100;return {raw,percent,discount,net:Math.max(0,raw-discount)}}
  function cbFractionalUnit(unit){return /^(KG|KGS|KILOGRAM|KILOGRAMS|GM|G|GRAM|GRAMS|LTR|LITER|LITRE|L|ML|MTR|METER|METRE|CM)$/i.test(String(unit||'').trim())}
  function cbQtyStep(unit){return cbFractionalUnit(unit)?0.1:1}
  function cbSetQtyStep(el,unit){if(!el)return;const s=cbQtyStep(unit);el.step=String(s);el.min=s===1?'1':'0.001';el.dataset.qtyStep=String(s)}
  window.cbEditUomStep=function(unit){cbSetQtyStep(document.querySelector('#cbEditQty'),unit)};
  function applyCbChoice(choice){if(!choice||!selectedUom)return;const p=selectedUom.product;document.querySelector('#cbRate').value=Number(cbGrossRate(p,choice)||0).toFixed(2);document.querySelector('#cbMrp').value=Number(choice.mrp||0).toFixed(2);cbSetQtyStep(document.querySelector('#cbQty'),choice.unit);const av=Math.max(0,(Number(p.Stock||0)-cbBaseUsed(p.Id))/choice.factor);document.querySelector('#cbAvl').textContent=Number(av.toFixed(3));document.querySelector('#cbAvl').parentElement.title='Base stock: '+cbStockText(p.Stock,selectedUom.uom)}
  ...
  function lineTaxParts(x){
    const dl=cbLineDiscount(x),amount=dl.net,rate=Math.max(0,Number(x.Gst||0)),inclusive=String(x.TaxMode||'EXCLUSIVE').toUpperCase()==='INCLUSIVE';
    const tax=rate<=0?0:(inclusive?amount*rate/(100+rate):amount*rate/100);
    const taxable=inclusive?amount-tax:amount;
    return {taxable,tax,gross:inclusive?amount:amount+tax,lineDiscount:dl.discount,raw:dl.raw};
  }
  ...
  function totals(){
    ...
    const lineDiscount=parts.reduce((a,x)=>a+x.lineDiscount,0);
    return {sub,tax,discount:d.amount,lineDiscount,totalDiscount:lineDiscount+d.amount,total:Math.max(0,gross-d.amount)};
  }
  ...
  async function renderCounterBilling(){
    ...
    app.innerHTML=`<div class="counter-billing">
      ...
      <div class="cb-inputbar">
        ...
        <label class="cb-field">Quantity<input id="cbQty" type="number" min="1" step="1" value="1"></label>
        <label class="cb-field">Rate / MRP<input id="cbRate" type="number" step="0.01" value="0"></label>
  ...
  window.cbAddSelected=function(){
    ...
    if(c)c.Qty+=qty;
    else state.cart.push({Id:p.Id,Name:p.Name,Barcode:p.Barcode||p.Sku||p.Id,Qty:qty,Rate:rate,Gst:Number(p.GstRate||0),TaxMode:String(p.TaxMode||'EXCLUSIVE').toUpperCase(),Mrp:mrp,DiscountPer:cbProductDiscount(p),Uom:choice.unit,BaseUnit:selectedUom.uom.BaseUnit||p.Unit||'PCS',Factor:factor,Stock:Number(p.Stock||0)});
  ...
  window.cbEdit=async function(i){
    selectedRow=i;const c=state.cart[i];if(!c)return;const p=state.products.find(x=>x.Id===c.Id),u=await loadCbUom(c.Id,p),options=cbUnitChoices(p,u);
    modal('Edit Item · '+c.Name,`<div class="cb-modal-edit"><label>UOM<select id="cbEditUom" class="select" onchange="cbEditUomStep(this.value)">${options.map(o=>`<option value="${esc(o.unit)}" ${o.unit===c.Uom?'selected':''}>${esc(o.unit)} ×${o.factor}</option>`).join('')}</select></label><label>Quantity<input id="cbEditQty" class="input" type="number" min="${cbQtyStep(c.Uom)===1?'1':'.001'}" step="${cbQtyStep(c.Uom)}" value="${c.Qty}"></label><label>Rate / MRP<input id="cbEditRate" class="input" type="number" min="0" step=".01" value="${c.Rate}"></label><label>MRP<input id="cbEditMrp" class="input" type="number" min="0" step=".01" value="${c.Mrp}"></label><label>Discount %<input id="cbEditDiscount" class="input" type="number" min="0" max="100" step=".01" value="${Number(c.DiscountPer||0)}"></label><div class="muted">0% par discount line hide rahegi. Discount item amount par apply hoga.</div></div>`,`<button class="btn" onclick="cbApplyLineEdit(${i})">Apply Item Changes</button>`);
    setTimeout(()=>{document.querySelector('#cbEditDiscount')?.focus();document.querySelector('#cbEditDiscount')?.select()},40)
  };
  window.cbApplyLineEdit=async function(i){
    const c=state.cart[i];if(!c)return;const p=state.products.find(x=>x.Id===c.Id),u=await loadCbUom(c.Id,p),options=cbUnitChoices(p,u),unit=document.querySelector('#cbEditUom')?.value,o=options.find(x=>x.unit===unit)||options[0];
    const qty=Math.max(.001,Number(document.querySelector('#cbEditQty')?.value)||.001),rate=Math.max(0,Number(document.querySelector('#cbEditRate')?.value)||0),mrp=Math.max(0,Number(document.querySelector('#cbEditMrp')?.value)||0),discount=Math.max(0,Math.min(100,Number(document.querySelector('#cbEditDiscount')?.value)||0));
    const used=cbBaseUsed(c.Id,i);if(used+qty*o.factor>Number(c.Stock||p?.Stock||0))return toast('Stock limit reached');
    Object.assign(c,{Qty:qty,Rate:rate,Mrp:mrp,DiscountPer:discount,Uom:o.unit,Factor:o.factor,BaseUnit:u.BaseUnit||p?.Unit||'PCS'});closeModal();cbRender();document.querySelector('#cbSearch')?.focus()
  };

  function cbRender(){const rows=document.querySelector('#cbRows');if(!rows)return;rows.innerHTML=state.cart.map((x,i)=>{const dl=cbLineDiscount(x),disc=dl.percent>0?`<small class="cb-line-discount">Discount: ${Number(dl.percent.toFixed(2))}%</small>`:'';return `<tr class="${selectedRow===i?'selected':''}" onclick="cbSelectRow(${i})"><td>${esc(x.Barcode)}</td><td class="name"><b>${esc(x.Name)}</b>${disc}</td><td>${money(x.Rate)}</td><td><b>${esc(x.Uom)}</b><br><small>×${x.Factor} ${esc(x.BaseUnit)}</small></td><td><input class="qty" type="number" min="${cbQtyStep(x.Uom)===1?'1':'.001'}" step="${cbQtyStep(x.Uom)}" value="${x.Qty}" onclick="event.stopPropagation()" oninput="cbQty(${i},this.value)"></td><td>${money(dl.net)}</td><td>${money(x.Mrp)}</td><td><button class="cb-edit" title="Edit quantity, rate, MRP and discount" onclick="event.stopPropagation();cbEdit(${i})">✎</button></td></tr>`}).join('')||'<tr><td colspan="8" class="empty">No items in invoice</td></tr>';const t=totals(),d=discountInfo();document.querySelector('#cbItemCount').textContent=state.cart.length;document.querySelector('#cbQtyTotal').textContent=state.cart.reduce((a,x)=>a+x.Qty,0).toFixed(2);document.querySelector('#cbSub').textContent='₹'+money(t.sub);document.querySelector('#cbDisc').textContent='₹'+money(t.totalDiscount);document.querySelector('#cbDiscLabel').textContent=t.lineDiscount>0?('Items ₹'+money(t.lineDiscount)+(t.discount>0?' + Bill ₹'+money(t.discount):'')):(d.type==='PERCENT'?d.value+'%':'₹'+money(d.value));document.querySelector('#cbTax').textContent='₹'+money(t.tax);document.querySelector('#cbTotal').textContent='₹'+money(t.total);document.querySelector('#cbTender').textContent=paymentMode+(paymentMode==='Credit/UPI'?' · '+paymentSubtype:'');document.querySelector('#cbPaymentLabel').textContent=paymentMode+(paymentMode==='Credit/UPI'?' · '+paymentSubtype:'');document.querySelector('#cbBalance').textContent='₹0.00'}
  ...
  window.cbComplete=async function(){if(completing)return;if(!state.cart.length)return toast('Add item first');const selectedPrintMode=typeof billingGetPrintMode==='function'?await billingGetPrintMode():undefined,d=discountInfo(),t=totals();if(paymentMode==='Multi Mode'&&Math.abs(multiPayments.reduce((a,x)=>a+x.Amount,0)-t.total)>.01)return cbOpenMulti();if(paymentMode==='Credit/UPI'&&!multiPayments.length)multiPayments=[{Mode:'Credit/UPI',Type:paymentSubtype||'UPI',Amount:t.total,ReferenceNo:null}];if(paymentMode==='Cash')multiPayments=[{Mode:'Cash',Type:'Cash',Amount:t.total,ReferenceNo:null}];if(paymentMode==='BTC')multiPayments=[{Mode:'BTC',Type:'BTC',Amount:t.total,ReferenceNo:null}];completing=true;try{const customer=document.querySelector('#cbCustomer').value||'Walk-in Customer';const paid=multiPayments.filter(x=>x.Type!=='Credit').reduce((a,x)=>a+x.Amount,0);const data=await api('/api/premium/sales',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({CustomerId:null,CustomerName:customer,CustomerPhone:document.querySelector('#cbMobile')?.value?.trim()||null,CustomerGstIn:document.querySelector('#cbGstNo')?.value?.trim()||null,PaymentMode:paymentMode,PaidAmount:paid,DiscountType:d.type,DiscountValue:d.value,Notes:document.querySelector('#cbRemarks').value||null,Payments:multiPayments,Lines:state.cart.map(x=>({ProductId:x.Id,Qty:x.Qty*x.Factor,SalePrice:x.Rate/x.Factor,TaxRate:x.Gst,TaxMode:x.TaxMode||'EXCLUSIVE',Discount:cbLineDiscount(x).discount,UnitSold:x.Uom,SoldQty:x.Qty,BaseQty:x.Qty*x.Factor,RatePerSoldUnit:x.Rate}))})});toast('Bill completed: '+data.invoiceNo);if(typeof printLastBill==='function')await printLastBill(data.id,selectedPrintMode);setTimeout(renderCounterBilling,180)}catch(e){completing=false;alert(e.message)}};

  function cbKeys(e){if(state.page!=='billing'||!document.querySelector('.counter-billing'))return;const tag=(document.activeElement?.tagName||'').toLowerCase();if(e.key==='F1'){e.preventDefault();cbDiscount();return}if(e.key==='F2'){e.preventDefault();cbCycleUom();return}if(e.key==='F3'){e.preventDefault();document.querySelector('#cbMobile')?.focus();return}if(e.key==='F4'){e.preventDefault();cbReset();return}if(e.key==='F6'){e.preventDefault();cbPaySelect('Cash');return}if(e.key==='F7'){e.preventDefault();cbOpenCreditUpi();return}if(e.key==='F8'){e.preventDefault();cbPaySelect('BTC');return}if(e.key==='F9'){e.preventDefault();cbOpenMulti();return}if(e.key==='F10'){e.preventDefault();cbComplete();return}if(e.key==='Escape'){e.preventDefault();loadDashboard();return}if(tag==='input'||tag==='select'||tag==='textarea')return;if(e.key==='Delete'){e.preventDefault();cbRemoveSelected();return}if((e.key==='+'||e.key==='=')&&selectedRow>=0&&state.cart[selectedRow]){e.preventDefault();const c=state.cart[selectedRow],step=cbQtyStep(c.Uom);if(cbBaseUsed(c.Id,selectedRow)+(c.Qty+step)*c.Factor<=c.Stock)c.Qty=Number((c.Qty+step).toFixed(3));else toast('Stock limit reached');cbRender();return}if(e.key==='-'&&selectedRow>=0&&state.cart[selectedRow]){e.preventDefault();const c=state.cart[selectedRow],step=cbQtyStep(c.Uom);c.Qty=Math.max(step===1?1:.001,Number((c.Qty-step).toFixed(3)));cbRender()}}
