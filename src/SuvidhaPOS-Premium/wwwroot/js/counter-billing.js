(function(){
  ...
  function applyCbChoice(choice){if(!choice||!selectedUom)return;const p=selectedUom.product;document.querySelector('#cbRate').value=Number(cbGrossRate(p,choice)||0).toFixed(2);document.querySelector('#cbMrp').value=Number(choice.mrp||0).toFixed(2);const av=Math.max(0,(Number(p.Stock||0)-cbBaseUsed(p.Id))/choice.factor);document.querySelector('#cbAvl').textContent=Number(av.toFixed(3));document.querySelector('#cbAvl').parentElement.title='Base stock: '+cbStockText(p.Stock,selectedUom.uom)}
  ...
  window.cbEdit=async function(i){
    ...
    modal('Edit Item · '+c.Name,`<div class="cb-modal-edit"><label>UOM<select id="cbEditUom" class="select">${options.map(o=>`<option value="${esc(o.unit)}" ${o.unit===c.Uom?'selected':''}>${esc(o.unit)} ×${o.factor}</option>`).join('')}</select></label><label>Quantity<input id="cbEditQty" class="input" type="number" min=".001" step=".001" value="${c.Qty}"></label><label>Rate / MRP<input id="cbEditRate" class="input" type="number" min="0" step=".01" value="${c.Rate}"></label><label>MRP<input id="cbEditMrp" class="input" type="number" min="0" step=".01" value="${c.Mrp}"></label><label>Discount %<input id="cbEditDiscount" class="input" type="number" min="0" max="100" step=".01" value="${Number(c.DiscountPer||0)}"></label><div class="muted">0% par discount line hide rahegi. Discount item amount par apply hoga.</div></div>`,`<button class="btn" onclick="cbApplyLineEdit(${i})">Apply Item Changes</button>`);
