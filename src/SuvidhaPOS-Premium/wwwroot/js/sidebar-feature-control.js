...
const catalog=[
 ...
 ['JEWELLERY_ITEM_MASTER','Jewellery Item Master',['[data-page="jItemMaster"]'],['Jewellery Item Master','Item Master','Item Entry']]
];
...
function matchesText(el,names){
 ...
 const t=String(label?label.textContent:el.textContent||'').replace(/^[^\p{L}\p{N}]+/u,'').replace(/\s+/g,' ').trim().toLowerCase();
 return names.some(n=>t===String(n).toLowerCase())
}
function relevantFeature(key){
 const jewellery=d.body.classList.contains('jewel-suite-mode');
 return key==='ITEM_MASTER'?!jewellery:key==='JEWELLERY_ITEM_MASTER'?jewellery:true;
}
function apply(){
 if(!loaded)return;
 const side=d.getElementById('sidebar');if(!side)return;
 catalog.forEach(([key,,selectors,names])=>{
  if(!relevantFeature(key))return;
...
w.loadPremiumFeatureControl=async function(){
 ...
 const relevant=catalog.filter(([key,,selectors,names])=>{if(!relevantFeature(key))return false;if(['GIRVI','CRDR','SAVING','STOCK','JEWELLERY_ITEM_MASTER'].includes(key))return d.body.classList.contains('jewel-suite-mode')||textButtons().some(x=>matchesText(x,names));return true});
