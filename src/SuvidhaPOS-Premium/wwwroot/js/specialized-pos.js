(function(){
  ...
  async function postPurchase(lines,prefix,note){
    if(!lines.length)return toast('Add purchase items');
    const pharma=/pharmacy|medical/i.test(S.spec?.StoreType||'');
    if(pharma&&lines.some(x=>!String(x.BatchNo||'').trim()))return toast('Batch No is mandatory for Pharmacy / Medical Store');
    if(lines.some(x=>!x.ExpiryDate))return toast('Expiry date is required for every batch');
    ...
    await api('/api/purchases',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)})
  }
  window.saveUomPurchase=async function(){try{await postPurchase(S.purLines,'u','Multi-unit purchase; inventory posted in base units');if(!S.purLines.length)return;closeModal();toast('Purchase saved — stock converted to base units');loadPurchase()}catch(e){alert(e.message||e)}};
  ...
  function renderBarcodePurchaseLines(){
    ...
    el.innerHTML=S.barPurLines.map((x,i)=>`<div class="purchase-line-card barcode-line" data-index="${i}"><div class="purchase-line-head"><div><b>${esc2(x.Name)}</b><small>▥ ${esc2(x.Barcode||'-')} · Base ${esc2(x.Uom.BaseUnit)}</small></div><button class="btn small danger" onclick="S.barPurLines.splice(${i},1);renderBarcodePurchaseLines()">×</button></div><div class="purchase-line-fields"><label>Unit<select data-pur-key="1" class="select" onchange="barcodePurchaseUom(${i},this.value)">${x.Options.map(o=>`<option value="${esc2(o.unit)}" ${o.unit===x.Unit?'selected':''}>${esc2(o.unit)} ×${o.factor}</option>`).join('')}</select></label><label>Qty<input data-pur-key="1" class="input" type="number" min=".001" step=".001" value="${x.Qty}" oninput="barcodePurchaseSet(${i},'Qty',this.value)"></label><label>Free<input data-pur-key="1" class="input" type="number" min="0" step=".001" value="${x.FreeQuantity}" oninput="barcodePurchaseSet(${i},'FreeQuantity',this.value)"></label><label>Purchase<input data-pur-key="1" class="input" type="number" min="0" step=".01" value="${x.Cost}" oninput="barcodePurchaseSet(${i},'Cost',this.value)"></label><label>Sale<input data-pur-key="1" class="input" type="number" min="0" step=".01" value="${x.SalePrice}" oninput="barcodePurchaseSet(${i},'SalePrice',this.value)"></label><label>MRP<input data-pur-key="1" class="input" type="number" min="0" step=".01" value="${x.Mrp}" oninput="barcodePurchaseSet(${i},'Mrp',this.value)"></label><label>Batch<input data-pur-key="1" class="input" value="${esc2(x.BatchNo)}" oninput="barcodePurchaseSet(${i},'BatchNo',this.value)"></label><label>Expiry<input data-pur-key="1" class="input" type="date" value="${x.ExpiryDate}" onchange="barcodePurchaseSet(${i},'ExpiryDate',this.value)"></label></div><div class="purchase-line-summary" id="bpSummary${i}">${lineSummary(x)}</div></div>`).join('')
  }
  window.renderBarcodePurchaseLines=renderBarcodePurchaseLines;
  window.saveBarcodePurchase=async function(){try{await postPurchase(S.barPurLines,'bp','Barcode purchase; keyboard/scanner entry; inventory posted in base units');if(!S.barPurLines.length)return;closeModal();toast('Barcode Purchase saved and stock updated');loadPurchase()}catch(e){alert(e.message||e)}};
