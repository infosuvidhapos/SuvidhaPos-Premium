...
`;
 ...
 function buildHtml(d,mode=state.mode,template=state.template,width=state.width,autoPrint=false,currencyMode=state.currency){
   ...
   const rows=lines.map((x,i)=>{const z=lineInfo(x),discountPct=z.amount>0?z.discount*100/z.amount:0,hasSaving=template==='T11'&&z.discount>0,saving=hasSaving?`<tr class="saving-row"><td colspan="4"><div class="saving-line"><span>Discount: ${pct(discountPct)}%</span><span>You Save: ${ccy}${money(z.discount)}</span></div></td></tr>`:'';return `<tr class="${hasSaving?'discounted':''}">
